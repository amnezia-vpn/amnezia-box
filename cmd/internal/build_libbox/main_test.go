...
func TestMobileBuildArguments(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			debugEnabled = test.debug
			withTailscale = test.tailscale
			platform = test.platform
			extraTags = "with_awg,custom_feature"
			outputPath = "artifacts/custom.aar"
			configureBuild("1.260910.0")
			args := androidArgs()
			if test.apple {
				outputPath = "artifacts/custom.xcframework"
				args = appleArgs()
			...
			if got := argumentValue(t, args, "-o"); got != outputPath {
				t.Fatalf("output = %q, want %q", got, outputPath)
			}
			tags := strings.Split(argumentValue(t, args, "-tags"), ",")
			for _, required := range []string{"with_gvisor", "with_wireguard", "with_conntrack", "with_awg", "custom_feature"} {
